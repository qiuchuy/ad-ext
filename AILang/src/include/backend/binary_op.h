...
// tensor in this section have been broadcasted . so we dont need to

// void get_binary_op_output(const Array &a, const Array &b, Array &out,
//                           BinaryNodeType nodetype) {
//     switch (nodetype) {
//     // TODO need add the flag which decides the array's buffer wthether can
//     be
//     // share.
...
void binary(const Array &lhs, const Array &rhs, Array &out, Ops... ops) {
    Dtype output_type = lhs.dtype() < rhs.dtype() ? lhs.dtype() : rhs.dtype();
    switch (output_type.type) {
    case Dtype::DataType::Any:
        throw std::invalid_argument("[binary] not support Any type.");
        break;
    case Dtype::DataType::BoolType:
        binary_op<bool>(lhs, rhs, out, ops...);
        break;
    case Dtype::DataType::Int8Type:
        binary_op<uint8_t>(lhs, rhs, out, ops...);
        break;
    case Dtype::DataType::Int16Type:
        binary_op<uint16_t>(lhs, rhs, out, ops...);
        break;
    case Dtype::DataType::Int32Type:
        binary_op<uint32_t>(lhs, rhs, out, ops...);
        break;
    case Dtype::DataType::Int64Type:
        binary_op<uint64_t>(lhs, rhs, out, ops...);
        break;
    case Dtype::DataType::Float32Type:
        binary_op<float>(lhs, rhs, out, ops...);
        break;
    case Dtype::DataType::Float64Type:
        binary_op<double>(lhs, rhs, out, ops...);
        break;
    default:
        break;
    }
