...
class Tracer : public std::enable_shared_from_this<Tracer> {
  public:
    ...
    std::vector<std::shared_ptr<Tracer>> outputs() {
        ...
        outputs.insert(outputs.end(), siblings_.begin(),
                       siblings_.begin() + idx_);
        outputs.push_back(shared_from_this());
        outputs.insert(outputs.end(), siblings_.begin() + idx_,
                       siblings_.end());
        return outputs;
    }
...
class Array : public Tracer {
  public:
    ...
    template <typename T> T item() {
        ...
        if (ndim() != 0) {
            throw std::runtime_error(
                "Item() is only supported for scalar array.");
        }
        return *(data<T>());
    }
    ...
    bool operator==(Array &other) {
        ...
        bool strideEqual = std::equal(stride_->begin(), stride_->end(),
                                      other.stride_->begin());
