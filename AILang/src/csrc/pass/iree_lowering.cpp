...
int main(int argc, char **argv) {
    ...
    if (api_version_major > IREE_COMPILER_EXPECTED_API_MAJOR ||
        ...
        fprintf(stderr,
                ...
                IREE_COMPILER_EXPECTED_API_MINOR, api_version_major,
                api_version_minor);
        ireeCompilerGlobalShutdown();
        return 1;
    }
    ...
    // ------------------------------------------------------------------------
    // // Initialization and version checking complete, ready to use the
    // compiler. //
    // ------------------------------------------------------------------------
    // //
    ...
    // A session represents a scope where one or more invocations can be
    // executed.
...
func.func @simple_mul(%lhs: tensor<4xf32>, %rhs: tensor<4xf32>) -> tensor<4xf32> {\n\
  ...
  return %result : tensor<4xf32>\n \
    ...
    error = ireeCompilerSourceWrapBuffer(
        s.session, "simple_mul", simple_mul_mlir, strlen(simple_mul_mlir) + 1,
