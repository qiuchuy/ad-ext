...
void initAST(py::module_ &m) {
  ...
  py::class_<BindNode, StmtNode, std::shared_ptr<BindNode>>(m, "BindNode",
                                                            py::dynamic_attr())
      .def(py::init<>())
  ...
  py::class_<BinaryOpNode, ExprNode, std::shared_ptr<BinaryOpNode>>(
      ...
      .def(py::init<>())
      .def(
          py::init([](const std::string &op, const Expr &op1, const Expr &op2) {
            return std::make_shared<BinaryOpNode>(
                ainl::core::BinaryOpASTHelper(op), op1, op2);
          }))
      .def(py::init([](const std::string &op, const Expr &op1, const Expr &op2,
                       const TypePtr &type) {
  ...
  py::class_<VarDefNode, ExprNode, std::shared_ptr<VarDefNode>>(m, "VarDefNode")
      .def(py::init<>())
      .def(py::init<std::string>());
  ...
  py::class_<FunctionDefNode, StmtNode, std::shared_ptr<FunctionDefNode>>(
      ...
      .def(py::init<>())
      .def(
          py::init<std::string, std::vector<std::string>, std::vector<Stmt>>());
  ...
  py::class_<CompareNode, ExprNode, std::shared_ptr<CompareNode>>(m,
                                                                  "CompareNode")
      .def(py::init<>())
  ...
  py::class_<CallNode, ExprNode, std::shared_ptr<CallNode>>(m, "CallNode",
                                                            py::dynamic_attr())
      .def(py::init<>())
