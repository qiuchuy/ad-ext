        ...
        def _unpack_params(vk,v):
            ...
            if isinstance(v,list):
                ...
                for i,vi in enumerate(v):
                    prefix = f"{vk}.{i}"
                    next_list.append(_unpack_params(prefix,vi) if filter_fn(self,prefix,vi) else {})
                return next_list
        
            raise RuntimeError("Unexpected leaf found while traversing the module")
        return {k: _unpack_params(k, v) for k, v in self.__dict__.items() if filter_fn(self, k, v)}
    
    def _child_modules(self,value):
        return self.module_filter(
            self.valid_module_filter,
            isleaf_fn=lambda m,k,v:isinstance(v, (Module))
        )
