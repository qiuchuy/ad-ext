...
def ailang_convolution_gpu(
    ...
    input_data = al.from_numpy(input_data, device="gpu")
    weight = al.from_numpy(weight, device="gpu")
    conv_Layer = ailang.nn.Conv2d(
        3, 2, stride, kernel_size, lhs_dilation, 1, 0, padding
    )
    conv_Layer.weight = weight
...
if __name__ == "__main__":
    ...
    Colors.print_color(f"Begin Check: ", Colors.GREEN)
    delimiter("*", color=Colors.GREEN)
    ailang_res = ailang_convolution_gpu(
        input_data,
        weight,
        args.kernel_size,
        args.stride,
