...
def test_resnet():
    ...
    a_model.eval()
    model.eval()
    a_out = a_model.forward(a)
    t_out = model.forward(t)
    return a_out, t_out, numeric_check(a_out, t_out.detach().numpy())
...
if __name__ == "__main__":
    ...
    center("[CRITERION 3.1] Dynamic Graph - ResNet", Colors.GREEN)
    delimiter("=", color=Colors.GREEN)

    Colors.print_color("Test Illustration", Colors.GREEN)
    Colors.print_color(f"    test network: Dynamic Graph - ResNet", Colors.GREEN)
