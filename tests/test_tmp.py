...
class TestOP:
    ...
    def test_unary_op_maxpool(self):
        ...
        iree_result = al.standard.maxpool2d(
            y,
            (1, 1, 3, 3),
            (1, 1, 2, 2),
            (1, 1, 1, 1),
    ...
    def test_unary_op_conv(self):
        a = np.random.randn(1, 1, 224, 224).astype(np.float32)
        b = np.random.randn(1, 3, 3, 1).astype(np.float32)
        c = al.from_numpy(a)
        d = al.from_numpy(b)
        iree_result = al.standard.conv2d(
            c, d, (4, 4), (2, 2), (1, 1), (0, 0, 0, 0), (0, 0)
        )
        print("Result: ", iree_result)
