...
x_torch = torch.tensor(x.flatten(), dtype=torch.float32, requires_grad=True).unsqueeze(0)
h_prev_torch = torch.tensor(h_prev.flatten(), dtype=torch.float32, requires_grad=True).unsqueeze(0)
c_prev_torch = torch.tensor(c_prev.flatten(), dtype=torch.float32, requires_grad=True).unsqueeze(0)
x_torch.retain_grad()
...
al_hc_t_sum, grad_x_t, grad_h_prev, grad_c_prev = al_lstm_cell.forward(al_x_t, al_h_prev, al_c_prev)
