...
if __name__ == '__main__':
    ...
    with torch.no_grad():
        ...
        model.eval()
        # data = torch.randn(1, 3, 20, 20)
        inp = torch.randn([seq_len, 1, input_size])
        out_stablehlo_mlir_path = "./lstm.mlir"

        module = torchscript.compile(
            model, inp, output_type=torchscript.OutputType.STABLEHLO, use_tracing=False
        )
        with open(out_stablehlo_mlir_path, "w", encoding="utf-8") as outf:
            outf.write(str(module))

        print(f"TOSA IR of resent18 successfully written into {out_stablehlo_mlir_path}")
